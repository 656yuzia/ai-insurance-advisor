...
- 禁止使用「一定」、「勢必」、「高機率」、「很危險」、「必須立刻」、「明顯不足」、「嚴重缺口」。
...
【優先確認項目】
如果有回答「沒有」或「不確定」，最多列 2 項，每項最多 1 句且不超過約 45 個中文字，只挑最有關聯的方向。不要使用編號或條列符號；每項以「項目名稱：」開頭。如果輸出 2 項，兩項中間必須剛好空一行，形成兩個短段落，不可連續堆疊或加入更多空白行。
...
整體硬性規則：
- 使用繁體中文，全文一律使用「您」，不得使用「你」。
- 先肯定使用者已經完成整理或已有部分安排，再協助確認最重要的 1～2 個方向；內容是整理與確認，不是診斷與判定。
...
- 必須符合 ai-brain 裡「我的銷售風格.md」。`
    ...
    const gapAnalysis = yesCount === 4 ? applyAllCoveredSummary(response.output_text) : response.output_text;
